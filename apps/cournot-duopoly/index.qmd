...
sliderInput("a", label = "Demand Intercept (a):", 
            min = 12, max = 15, value = 12, step = 0.5)

sliderInput("b", label = "Demand Slope (b):", 
            min = 1, max = 5, value = 1, step = 0.5)

sliderInput("ci", label = "Marginal Cost Firm i (ci):", 
            min = 0, max = 5, value = 0, step = 0.5)

sliderInput("cj", label = "Marginal Cost Firm j (cj):", 
            min = 0, max = 5, value = 0, step = 0.5)
...
sliderInput("ki", label = "Weight Firm i (ki):", 
            min = 0, max = 1, value = 0, step = 0.1)

sliderInput("kj", label = "Weight Firm j (kj):", 
            min = 0, max = 1, value = 0, step = 0.1)
...
tabsetPanel(
  ...
  tabPanel("Mathematical Details",
    ...
    p("$$p = a - b(q_i + q_j)$$"),
    p("The Best Response (Reaction) functions are derived from profit maximization:"),
    p("$$R_i(q_j): q_i = \\frac{a - c_i - b q_j}{2b}$$"),
    p("$$R_j(q_i): q_j = \\frac{a - c_j - b q_i}{2b}$$"),
    
    h4("Key Points"),
    tags$ul(
      tags$li("The ", strong("Cournot Equilibrium"), " (Green Dot) is the intersection of the two reaction curves."),
      tags$li("The ", strong("Monopoly Points"), " represent where a firm would produce if the rival produced 0.")
    ),
    
    h4("Equilibrium Quantities"),
    p("Solving the system yields the Cournot quantities:"),
    p("$$q_i^C = \\frac{a - 2c_i + c_j}{3b}, \\quad q_j^C = \\frac{a - 2c_j + c_i}{3b}$$")
  )
)
```
...
output$cournotPlot <- renderPlot({
  ...
  ggplot() +
    ...
    # Iso-profit Contours
    geom_contour(data = grid, aes(x = qi, y = qj, z = pi_i), 
                 breaks = target_pi_i, color = "blue", linetype = "dotted", size = 0.8) +
    geom_contour(data = grid, aes(x = qi, y = qj, z = pi_j), 
                 breaks = target_pi_j, color = "red", linetype = "dotted", size = 0.8) +
    
    # Reaction Functions
    # We plot these using stat_function for smoothness
    # Firm i Best Response (Blue)
    stat_function(fun = function(x) (a - ci - b*x)/(2*b), 
                  aes(y = x), # Note: we are flipping axes for Ri logic, but ggplot expects y=f(x)
                  xlim = c(0, 10), color = "blue", size = 1.2, alpha=0.3) + 
    # Actually, simpler to draw Ri as qj = (a-ci)/b - 2qi
    stat_function(fun = function(x) (a - ci)/b - 2*x, 
                  xlim = c(0, 10), color = "blue", size = 1.2) +
    
    # Firm j Best Response (Red)
    stat_function(fun = function(x) (a - cj - b*x)/(2*b), 
                  xlim = c(0, 10), color = "red", size = 1.2) +
    ...
    # Labels
    annotate("text", x = 9, y = (a - ci)/b - 18, label = TeX("$R_i(q_j)$"), color = "blue") +
    annotate("text", x = 9, y = (a - cj - b*9)/(2*b) + 0.5, label = TeX("$R_j(q_i)$"), color = "red") +
