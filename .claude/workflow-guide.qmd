...
/proofread lecture-slides/slides/lecture-1-oligopoly.qmd

# Test 3: Does the quality score work?
python scripts/quality_score.py lecture-slides/slides/lecture-1-oligopoly.qmd
```
...
::: {.callout-important}
## Hard Rule: Don’t Double-Run Full Review Stacks in Orchestrator Mode

Once a plan is approved, you are in **orchestrator mode**: the orchestrator already runs the standard review agents (proofreader + slide-auditor + pedagogy-reviewer) as part of its verify–review–fix loop.

Do **not** manually run a second full review stack (e.g., `/slide-excellence`, or stacking `/proofread` + `/visual-audit` + `/pedagogy-review`) on the same revision unless you are intentionally doing a **milestone audit** to capture an independent review snapshot.
:::
...
/create-lecture
  ...
  +-- Phase 5: Review, polish, and verify
  |     +-- /slide-excellence (canonical release review)
  |     +-- /substance-review (optional: domain correctness)
  |     +-- /proofread OR /visual-audit OR /pedagogy-review (diagnostics-only, as needed)
  +-- Phase 6: Deploy
...
For lecture slide decks, the canonical release review is:

`/slide-excellence lecture-X-topic.qmd`

Use this instead of stacking `/proofread`, `/visual-audit`, and `/pedagogy-review`.

The canonical spec (agents + artifacts) lives in `.claude/skills/slide-excellence/SKILL.md`. At a high level:

```
/slide-excellence lecture-5-multi-stage-games.qmd
  |
  ...
  +-- Inline checks: citation key audit + parity checks (if applicable)
  |
...
- `/substance-review lecture-X-topic.qmd` for domain correctness (read-only)
- `/qa-quarto LectureX` for Quarto vs Beamer parity (translation mode)
