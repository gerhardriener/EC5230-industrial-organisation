...
# Product differentiation {background-color="#00539b"}
...
<!-- ```{r}
#| label: fig-salop-setting
#| fig-cap: "Salop's circular market: n=8 firms equally spaced around unit circle."
library(ggplot2)
...
firms$x <- cos(firms$ang)
firms$y <- sin(firms$ang)

ggplot() +
  geom_path(data = circle, aes(x, y), linewidth = 1) +
  geom_point(data = firms, aes(x, y), size = 3) +
  coord_equal() +
  theme_void()
``` -->
...
- **Firm choice in competitive region**  
  ...
  - Best response: $MR = MC$ ⇔ $q = \frac{\frac{c}{n} + \tilde{p} - m}{2c}$  
