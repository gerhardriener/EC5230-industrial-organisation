...
\pgfmathsetmacro{\QmO}{(\A-\cO)/2}  % 5
\pgfmathsetmacro{\PmO}{(\A+\cO)/2}  % 15
\pgfmathsetmacro{\QmI}{(\A-\cI)/2}  % 7
\pgfmathsetmacro{\PmI}{(\A+\cI)/2}  % 13
...
% Top strip: P^m(c0) to P^m(c1) is GONE (P^m(c1) < P^m(c0) since c1<c0... wait
% Actually P^m(c1) = 13, P^m(c0) = 15, so P^m(c1) < P^m(c0)
...
% Label gain
\node[font=\small, blue!70!black, fill=white, fill opacity=0.85, text opacity=1, inner sep=2pt] at (axis cs:3.5,8) {gain};
...
\pgfmathsetmacro{\cO}{10}
\pgfmathsetmacro{\cI}{6}
\pgfmathsetmacro{\Qpc}{\A - \cO}  % 10 (non-drastic: limit price at c0)
...
\node[font=\small, red!70!black, align=center] at (axis cs:12,11)
  {Pre-innovation:\\$\pi = 0$};
